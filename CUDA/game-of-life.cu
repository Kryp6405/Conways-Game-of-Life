...
__global__ void padded_matrix_copy(int *dst, int *src, int width, int height, int padding) {
    ...
    int dst_pitch = height + 2 * padding;
    int src_pitch = height;

    // Striding over the width and height
    for (int x = idx; x < width; x += gridx) {
        for (int y = idy; y < height; y += gridy) {
            int dst_index = (x + padding) * dst_pitch + (y + padding);
            int src_index = x * src_pitch + y;
...
int main(int argc, char *argv[]) {
    ...
    cout << "Runtime " << blockDimSize <<  ": " << elapsed/1000.0 << " s\n";
